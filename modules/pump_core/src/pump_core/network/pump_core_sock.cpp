...
#if (defined PUMP_OS_WINDOWS)
    pump_int32_t ret = ::closesocket((pump_sock_t)(long)m_handle);
#elif (defined PUMP_OS_POSIX)
    pump_int32_t ret = ::close((pump_sock_t)(long)m_handle);
#endif // (defined PUMP_OS_WINDOWS)
    if (ret == PUMP_OK)
    {
...
pump_int32_t CSock::Shutdown(pump_int32_t iHow)
{
    ...
    pump_int32_t ret = ::shutdown((pump_sock_t)(long)m_handle, iHow);
    if (ret == PUMP_OK)
    {
...
pump_int32_t CSock::Bind(const CAddr & refAddr)
{
    ...
    {
        return ::bind((pump_sock_t)(long)m_handle, (struct sockaddr *)&struAddr.sa_, sizeof(pump_sockaddr_in));
    }
    else
    {
        return ::bind((pump_sock_t)(long)m_handle, (struct sockaddr *)&struAddr.sa_, sizeof(PUMP_ADDR));
    }
}
...
pump_int32_t CSock::Listen(pump_int32_t iBackLog)
{
    ...
    pump_int32_t ret = ::listen((pump_sock_t)(long)m_handle, iBackLog);
    if (ret == PUMP_OK)
    {
...
CSock* CSock::Accept()
{
    ...
    hAcceptSock = (pump_handle_t)::accept((pump_sock_t)(long)m_handle, (sockaddr*)&struAddr.sa_, (socklen_t*)&dwAddrSize);
    CAddr objAddr;
...
CSock* CSock::AcceptExt(pump_uint32_t dwTimeOut)
{
    ...
    else
    {
        ...
        FD_ZERO(&fdset_r);
        FD_SET((pump_sock_t)(long)m_handle, &fdset_r);

        iSelectRet = PUMP_CORE_Select((pump_int32_t)(long)m_handle + 1, &fdset_r, NULL, NULL, &tv);
        if (iSelectRet > 0 && FD_ISSET((pump_sock_t)(long)m_handle, &fdset_r))
        {
            hAcceptSock = (pump_handle_t)::accept((pump_sock_t)(long)m_handle, (sockaddr*)&struAddr.sa_, (socklen_t*)&dwAddrSize);
        }
    }
...
pump_int32_t CSock::Connect(const CAddr & refAddr)
{
    ...
    memset(&struAddr, 0, sizeof(struAddr));
    refAddr.GetAddr(struAddr);
    pump_int32_t ret = ::connect((pump_sock_t)(long)m_handle, (struct sockaddr *)&struAddr.sa_, sizeof(pump_sockaddr_in));
    if (ret == PUMP_OK)
    {
...
pump_int32_t CSock::ConnectExt(const CAddr & refAddr, pump_uint32_t dwTimeOut)
{
    ...
    if (dwTimeOut==PUMP_INFINITE)
    {
        return ::connect((pump_sock_t)(long)m_handle, (struct sockaddr *)&struAddr.sa_, sizeof(pump_sockaddr_in));
    }
    ...
    {
        iRet = ::connect((pump_sock_t)(long)m_handle, (struct sockaddr *)&struAddr.sa_.sin4_, sizeof(struAddr.sa_.sin4_));
    }
    else
    {
        iRet = ::connect((pump_sock_t)(long)m_handle, (struct sockaddr *)&struAddr.sa_.sin6_, sizeof(struAddr.sa_.sin6_));
    }
...
#elif (defined PUMP_OS_POSIX)
    if (PUMP_CORE_GetSystemError() != EINPROGRESS)
#endif // PUMP_OS_WINDOWS
    {
        return iRet;
    }
    ...
    FD_ZERO(&fdset_w);
    FD_SET((pump_sock_t)(long)m_handle, &fdset_w);

    iSelectRet = PUMP_CORE_Select((pump_sock_t)(long)m_handle + 1, NULL, &fdset_w, NULL, &tv);
    if (iSelectRet > 0 && FD_ISSET((pump_sock_t)(long)m_handle, &fdset_w))
    {
        pump_int32_t iError = 0;
        pump_int32_t iErrorLen;
        iErrorLen = sizeof(iError);
        pump_int32_t iTmp = ::getsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_ERROR, (char*)iError, (socklen_t*)&iErrorLen);
        if ((iTmp == 0) && (iError == 0))
        {
            iRet = 0;
        }
...
pump_int32_t CSock::BroadCastOn()
{
    ...
    return ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_BROADCAST, (char*)&bSwitch, sizeof(bSwitch));
}
...
pump_int32_t CSock::JoinMultiCastGroup(const CAddr & refLocalAddr, const CAddr & refMCGroupAddr)
{
    ...
    struct sockaddr * pSockAddr = (struct sockaddr *)&(struMGAddr.sa_);
    if (pSockAddr->sa_family == PUMP_AF_INET)
    {
        ...
        return ::setsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&struMerq, sizeof(struMerq));
    }
    ...
    else if (pSockAddr->sa_family == PUMP_AF_INET6)
    {
        ...
        return ::setsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&struMerq6, sizeof(struMerq6));
    }
...
#ifdef PUMP_OS_WINDOWS
    return ::ioctlsocket((pump_sock_t)(long)m_handle, (pump_long_t)FIONBIO, (pump_ulong_t*)&bBlock);
#elif (defined PUMP_OS_POSIX)
    pump_int32_t iFlags = ::fcntl((pump_sock_t)(long)m_handle, F_GETFL, 0);
    if (bBlock)
    {
        return ((::fcntl((pump_sock_t)(long)m_handle, F_SETFL, iFlags | O_NONBLOCK) == -1) ? PUMP_ERROR:PUMP_OK);
    }
    else
    {
        return ((::fcntl((pump_sock_t)(long)m_handle, F_SETFL, iFlags & (~O_NONBLOCK)) == -1) ? PUMP_ERROR : PUMP_OK);
...
pump_int32_t CSock::GetTTL(pump_int32_t & iTTL)
{
    ...
    return ::getsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, IP_TTL, (char*)iTTL, (socklen_t*)&iSize);
}
...
pump_int32_t CSock::SetTTL(pump_int32_t iTTL)
{
    ...
    return ::setsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, IP_TTL, (char*)iTTL, sizeof(iTTL));
}
...
pump_int32_t CSock::SetTOS(pump_int32_t iTOS)
{
    ...
    return ::setsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, IP_TOS, (char*)iTOS, sizeof(iTOS));
}
...
pump_int32_t CSock::GetTOS(pump_int32_t & iTOS)
{
    ...
    return ::getsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, IP_TOS, (char*)iTOS, (socklen_t*)&iSize);
}
...
pump_int32_t CSock::SetReuseAddr(pump_bool_t bReuse)
{
    ...
    return ::setsockopt((pump_sock_t)(long)m_handle, IPPROTO_IP, SO_REUSEADDR, (char*)bReuse, sizeof(bReuse));
}
...
pump_int32_t CSock::SetTimeout(pump_int32_t iSendTimeout, pump_int32_t iRecvTimeout)
{
    ...
    if (iSendTimeout)
    {
        iRetSend = ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_SNDTIMEO, (char*)&iSendTimeout, sizeof(iSendTimeout));
    }
    if (iRecvTimeout)
    {
        iRetRecv = ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_RCVTIMEO, (char*)&iRecvTimeout, sizeof(iRecvTimeout));
    }
#elif (defined PUMP_OS_POSIX)
    if (iSendTimeout)
    {
        timeval tv = { iSendTimeout / 1000, iSendTimeout % 1000 };
        iRetSend = ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_SNDTIMEO, (char*)&tv, sizeof(tv));
    }
    if (iRecvTimeout)
    {
        timeval tv = { iSendTimeout / 1000, iSendTimeout % 1000 };
        iRetRecv = ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(tv));
    }
#endif // (defined PUMP_OS_WINDOWS)
...
pump_int32_t CSock::SetBuffSize(pump_int32_t iSendBuffSize, pump_int32_t iRecvBuffSize)
{
    ...
    if (iSendBuffSize)
    {
        iRetSend = ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_SNDBUF, (char*)&iSendBuffSize, sizeof(iSendBuffSize));
    }
    if (iRecvBuffSize)
    {
        iRetRecv = ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_RCVBUF, (char*)&iRecvBuffSize, sizeof(iRecvBuffSize));
    }
#endif // (defined PUMP_OS_WINDOWS)
    return ((iRetSend == PUMP_OK && iRetRecv == PUMP_OK) ? PUMP_OK : PUMP_ERROR);
}
...
pump_int32_t CSock::GetBuffSize(pump_int32_t & iSendBuffSize, pump_int32_t & iRecvBuffSize)
{
    ...
    if (iSendBuffSize)
    {
        iRetSend = ::getsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_SNDBUF, (char*)&iSendBuffSize, (socklen_t*)&iSizeSend);
    }
    if (iRecvBuffSize)
    {
        iRetRecv = ::getsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_RCVBUF, (char*)&iRecvBuffSize, (socklen_t*)&iSizeRecv);
    }
#endif // (defined PUMP_OS_WINDOWS)
    return ((iRetSend == PUMP_OK && iRetRecv == PUMP_OK) ? PUMP_OK : PUMP_ERROR);
}
...
pump_int32_t CSock::LingerOff()
{
    ...
    linger lin = { 0 };
    lin.l_onoff = 0;
    return ::setsockopt((pump_sock_t)(long)m_handle, SOL_SOCKET, SO_LINGER, (char*)&lin, sizeof(lin));
}
...
pump_int32_t CSock::SetNoDelay(pump_bool_t bDelay)
{
    ...
    return ::setsockopt((pump_sock_t)(long)m_handle, IPPROTO_TCP, TCP_NODELAY, (char*)&bDelay, sizeof(bDelay));;
}
...
pump_int32_t CSock::Send(const pump_pvoid_t pBuf, pump_uint32_t dwBufSize)
{
    ...
    return ::send((pump_sock_t)(long)m_handle, (char*)pBuf, dwBufSize, 0);
}
...
pump_int32_t CSock::Recv(pump_pvoid_t pBuf, pump_uint32_t dwBufSize)
{
    ...
    return ::recv((pump_sock_t)(long)m_handle, (char*)pBuf, dwBufSize, 0);
}
...
pump_int32_t CSock::SendTo(const pump_pvoid_t pBuf, pump_uint32_t dwBufSize, const CAddr & refLocalAddr)
{
    ...
    if (sin4.sin_family == PUMP_AF_INET)
    {
        return ::sendto((pump_sock_t)(long)m_handle, (char*)pBuf, dwBufSize, 0, (struct sockaddr *)&struAddr.sa_, sizeof(struAddr.sa_.sin4_));
    } 
    else
    {
        return ::sendto((pump_sock_t)(long)m_handle, (char*)pBuf, dwBufSize, 0, (struct sockaddr *)&struAddr.sa_, sizeof(struAddr.sa_.sin6_));
    }
}
...
pump_int32_t CSock::RecvFrom(pump_pvoid_t pBuf, pump_uint32_t dwBufSize, CAddr & refLocalAddr)
{
    ...
    return ::recvfrom((pump_sock_t)(long)m_handle, (char*)pBuf, dwBufSize, 0, (struct sockaddr *)&struAddr.sa_, (socklen_t*)&iAddrSize);
}
...
//#ifdef PUMP_OS_WINDOWS
//    return ::ioctlsocket((pump_sock_t)(long)m_handle,iCmd,(pump_ulong_t*))
//#elif (defined PUMP_OS_POSIX)
//#endif // PUMP_OS_WINDOWS
//    return ::ioctlsocket((pump_sock_t)(long)m_handle,iCmd,(pump_ulong_t*))
//#elif (defined PUMP_OS_POSIX)
...
{
    return (pump_sock_t)(long)m_handle;
}
