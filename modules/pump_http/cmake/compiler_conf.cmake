message("<pump::pump_http> /modules/pump_http/cmake/compiler_conf.cmake in")
set(pump_http_INNER_COMPILE_FLAG )
set(pump_http_INNER_LINK_FLAG )

# test code lpump_httpel: DEBUG(0) INFO(1) WARN(2) ERROR(3) CRITICAL(4) RELEASE(5)
set(debug_pump_http 0) # 用户设置编译选项
if(${debug_pump_http} LESS 1)
    set(Db_Flage -D_TEST_LEVEL_DEBUG -D_TEST_LEVEL_INFO -D_TEST_LEVEL_WARN -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_http} LESS 2)
    set(Db_Flage -D_TEST_LEVEL_INFO -D_TEST_LEVEL_WARN -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_http} LESS 3)
    set(Db_Flage -D_TEST_LEVEL_WARN -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_http} LESS 4)
    set(Db_Flage -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_http} LESS 5)
    set(Db_Flage -D_TEST_LEVEL_CRITICAL)
else(${debug_pump_http} LESS 6)
    set(Db_Flage )
endif()#include <boost\function.hpp>
message(">>> Debug: ${Db_Flage}")
set(pump_http_INNER_COMPILE_FLAG ${pump_http_INNER_COMPILE_FLAG} ${Db_Flage})
# add_compile_options(${Db_Flage} ${pump_http_INNER_COMPILE_FLAG})

if(${CMAKE_GENERATOR} STREQUAL "Unix Makefiles")
    set(pump_http_INNER_COMPILE_FLAG ${pump_http_INNER_COMPILE_FLAG}
            -Wl,-rpath=. 
            -Wno-write-strings 
            -fPIC)
    if(${pump_http_BUILD_FLAG} MATCHES "shared")
        set(pump_http_INNER_COMPILE_FLAG ${pump_http_INNER_COMPILE_FLAG}
            -DPUMP_HTTP_BUILD_DLL)
    endif()
    set(pump_http_INNER_LINK_FLAG ${pump_http_INNER_LINK_FLAG}
        -lpthread 
        -lrt)
