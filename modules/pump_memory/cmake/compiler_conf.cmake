message("<pump::pump_memory> /modules/pump_memory/cmake/compiler_conf.cmake in")
set(pump_memory_INNER_COMPILE_FLAG )
set(pump_memory_INNER_LINK_FLAG )

# test code lpump_memoryel: DEBUG(0) INFO(1) WARN(2) ERROR(3) CRITICAL(4) RELEASE(5)
set(debug_pump_memory 0) # 用户设置编译选项
if(${debug_pump_memory} LESS 1)
    set(Db_Flage -D_TEST_LEVEL_DEBUG -D_TEST_LEVEL_INFO -D_TEST_LEVEL_WARN -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_memory} LESS 2)
    set(Db_Flage -D_TEST_LEVEL_INFO -D_TEST_LEVEL_WARN -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_memory} LESS 3)
    set(Db_Flage -D_TEST_LEVEL_WARN -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_memory} LESS 4)
    set(Db_Flage -D_TEST_LEVEL_ERROR -D_TEST_LEVEL_CRITICAL)
elseif(${debug_pump_memory} LESS 5)
    set(Db_Flage -D_TEST_LEVEL_CRITICAL)
else(${debug_pump_memory} LESS 6)
    set(Db_Flage )
endif()
message("<pump::pump_memory> Db_Flage=${Db_Flage}")
set(pump_memory_INNER_COMPILE_FLAG ${pump_memory_INNER_COMPILE_FLAG} ${Db_Flage})
# add_compile_options(${Db_Flage} ${pump_memory_INNER_COMPILE_FLAG})

if(${CMAKE_GENERATOR} STREQUAL "Unix Makefiles")
    set(pump_memory_INNER_COMPILE_FLAG ${pump_memory_INNER_COMPILE_FLAG}
            -Wl,-rpath=. -Wno-write-strings -fPIC)
    if(${pump_memory_BUILD_FLAG} MATCHES "shared")
        set(pump_memory_INNER_COMPILE_FLAG ${pump_memory_INNER_COMPILE_FLAG}
                -DPUMP_MEMORY_BUILD_DLL 
                -DPUMP_CORE_USING_DLL)
    elseif(${pump_memory_BUILD_FLAG} MATCHES "static")
        set(pump_memory_INNER_COMPILE_FLAG ${pump_memory_INNER_COMPILE_FLAG}
                -DPUMP_MEMORY_BUILD_SLL 
                -DPUMP_CORE_USING_DLL)
    endif()
    set(pump_memory_INNER_LINK_FLAG ${pump_memory_INNER_LINK_FLAG}
        -lpthread -lrt)
...
elseif(${CMAKE_GENERATOR} MATCHES "Visual Studio")
    ...
    if(${pump_memory_BUILD_FLAG} MATCHES "shared")
        set(pump_memory_INNER_COMPILE_FLAG ${pump_memory_INNER_COMPILE_FLAG}
                -DPUMP_MEMORY_BUILD_DLL -DPUMP_CORE_USING_DLL)
    elseif(${pump_memory_BUILD_FLAG} MATCHES "static")
        set(pump_memory_INNER_COMPILE_FLAG ${pump_memory_INNER_COMPILE_FLAG}
                -DPUMP_MEMORY_BUILD_SLL -DPUMP_CORE_USING_DLL)
    endif()
...
message("<pump::pump_memory> /modules/pump_memory/cmake/compiler_conf.cmake out")